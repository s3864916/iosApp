...
class OrderModel:ObservableObject{
    ...
    func add(menuID:Int, size:Size = .small, quantity:Int = 1, comments:String = ""){
        ...
        let order = OrderItem(id: newId(), menuID: menuID, name: orderedItem.name, comments: comments, price: Double(orderedItem.price), size: size, quantity: quantity)
        orders += [order]
    }
